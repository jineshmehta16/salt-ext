...
var ComponentService = class _ComponentService {
  ...
  octokit;
  cache = {};
  examplesCache = /* @__PURE__ */ new Map();
  constructor() {
  }
  static getInstance() {
  ...
  async fetchComponentFile(component) {
    try {
  ...
  async fetchDocumentation(component) {
    try {
      await this.initOctokit();
      const componentLower = component.toLowerCase();
  ...
  parseProps(content) {
    ...
    return propsMatch[1].split(";").map((prop) => prop.trim()).filter(Boolean).map((prop) => {
  ...
  generateExample(component, props) {
    const importStatement = `import { ${component} } from "@salt-ds/core";`;
    const propsString = props.slice(0, 3).map((prop) => `${prop}={${typeof prop === "string" ? `"value"` : "value"}}`).join("\n  ");
...
const Example = () => (
  ...
  async getComponentInfo(component) {
    ...
    const info = {
      ...
      props,
      example,
      documentation: documentation || void 0
    };
...
// src/extension.ts
var componentService = ComponentService.getInstance();
async function handleSaltExplain(component) {
...
`;
  ...
  return `I couldn't find specific information about "${component}".${availableComponents.length ? `
...
function activate(context) {
  ...
  const saltParticipant = vscode.chat.createChatParticipant("salt", async (request2, context2, stream, token) => {
    const prompt = request2.prompt.toLowerCase();
...
Just ask me about a specific component or topic! ${availableComponents.length ? `
